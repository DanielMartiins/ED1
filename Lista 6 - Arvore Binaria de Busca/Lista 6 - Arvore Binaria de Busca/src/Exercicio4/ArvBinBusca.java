...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> {
    ...
    /*
        Complexidade alcançada: O(N²). O método delete() é O(N) e é chamado a cada vez que encontro um Nó para
        remover. E o meu método percorre toda a árvore, logo no pior caso eu posso acabar chamando o delete()
        para cada Nó da árvore, ficando O(N²)
     */

    public boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax) {
        return removeForaIntervalo(chaveMin, chaveMax, raiz);
    }

    private boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax, No no) {
        if (no == null) return false;

        int cmpChaveMin = no.chave.compareTo(chaveMin);
        int cmpChaveMax = no.chave.compareTo(chaveMax);
        boolean removeuPai = false;

        if (cmpChaveMin < 0 || cmpChaveMax > 0) {
            raiz = delete(raiz, no.chave);
            removeuPai = true;
        }
        boolean removeuEsq = removeForaIntervalo(chaveMin, chaveMax, no.esq);
        boolean removeuDir = removeForaIntervalo(chaveMin, chaveMax, no.dir);
        return removeuEsq || removeuDir || removeuPai;
    }
    ...
    Obs: "ref_no" � o ponteiro que referencia o n� para o qual est� sendo chamada a fun��o, o qual pode ter
    que ser modificado. Retorna false se o valor n�o pertencer � "�rvore".
     */
