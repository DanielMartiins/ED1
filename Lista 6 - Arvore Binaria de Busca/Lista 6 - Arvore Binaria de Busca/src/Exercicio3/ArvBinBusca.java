...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> {
    ...
    /*
    Complexidade alcançada: O(N). Todos nós são verificados uma única vez.
     */
    public boolean eSimilar(ArvBinBusca<Chave, Valor> arvore) {
        return eSimilar(this.raiz, arvore.raiz);
    }

    private boolean eSimilar(No no1, No no2) {
        if (no1 == null && no2 == null) return true;
        else if (no1 == null || no2 == null) return false;

        return eSimilar(no1.esq, no2.esq) && eSimilar(no1.dir, no2.dir);
    }
