...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> {
    ...
    /*
    Não faz sentido o protótipo do exercício, pois não faz sentido o método dado ser público
    se "No" é uma classe privada interna da classe ArvBinBusca. Portanto, considerei que, ao
    invés de receber e retornar um No, o método recebe duas Chaves e retorna uma Chave.

    Complexidade O(N). Contem() é O(N) e é chamado duas vezes e minha solução também percorre
    a árvore toda no pior caso, após certificar por meio do contem() que a chave existe.
    */
    public Chave obtemAncestralComum(Chave primeiraChave, Chave segundaChave) {

        if (primeiraChave == null || segundaChave == null)
            throw new IllegalArgumentException("A chave fornecida é null!");
        else if (!contem(primeiraChave) || !contem(segundaChave)) return null;

        No ancestral = obtemAncestralComum(raiz, primeiraChave, segundaChave);
        if (ancestral == null) return null;
        else return ancestral.chave;
    }

    private No obtemAncestralComum(No no, Chave primeiraChave, Chave segundaChave) {

        if (no == null) return null;

        int cmpPrimeiraChave = no.chave.compareTo(primeiraChave);
        int cmpSegundaChave = no.chave.compareTo(segundaChave);

        if (cmpPrimeiraChave < 0 && cmpSegundaChave < 0)
            return obtemAncestralComum(no.dir, primeiraChave, segundaChave);
        else if (cmpPrimeiraChave > 0 && cmpSegundaChave > 0)
            return obtemAncestralComum(no.esq, primeiraChave, segundaChave);
