...
public class HeapBinariaMinima {
    ...
    public HeapBinariaMinima(HeapBinariaMaxima heapMaxima) {
        ...
        ArrayList<Integer> vetorHeapMaxima = new ArrayList<>();
        vetorHeapMaxima.add(Integer.MIN_VALUE);
        for (int i = 0, maxHeap = heapMaxima.removeMax(); maxHeap != Integer.MIN_VALUE;
             i++, maxHeap = heapMaxima.removeMax()) {
            vetorHeapMaxima.add(heapMaxima.removeMax());
        }

        vetor = new int[vetorHeapMaxima.size() + 1];
        for (int i = 1; i < vetorHeapMaxima.size(); i++)
            vetor[i] = vetorHeapMaxima.get(i);
        constroiHeap();
    }
