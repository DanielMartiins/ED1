...
public class HeapBinariaMaxima {
    ...
    public HeapBinariaMaxima(HeapBinariaMinima heapMinima) {
        ...
        ArrayList<Integer> vetorHMinima = new ArrayList<>();
        vetorHMinima.add(Integer.MAX_VALUE);
        for (int i = 0, minHeap = heapMinima.removeMin(); minHeap != Integer.MAX_VALUE;
             i++, minHeap = heapMinima.removeMin()) {
            vetorHMinima.add(heapMinima.removeMin());
        }

        vetor = new int[vetorHMinima.size() + 1];
        for (int i = 1; i < vetorHMinima.size(); i++)
            vetor[i] = vetorHMinima.get(i);
        constroiHeap();
    }
