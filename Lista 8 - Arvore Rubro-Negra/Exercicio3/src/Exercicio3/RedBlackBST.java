...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    /*
     * Considerando que, em uma arvore rubro-negra, o caminho negro da raiz
     * para qualquer folha deve ser o mesmo, vou escolher ir da raiz para a
     * direita e contando a quantidade de nós negros.
     * Complexidade O(log(N)), pois na arvore rubro-negra a altura é menor
     * ou igual a 2*log(N) no pior caso.
     */
    public int alturaNegra() {
        if (root == null) return 0;
        return 1 + alturaNegra(root.right);
    }

    private int alturaNegra(Node h) {
        if (h == null) return 0;

        if (h.color == BLACK) return 1 + alturaNegra(h.right);
        else return alturaNegra(h.right);
    }
